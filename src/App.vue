  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">班型领域</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">教案/文案类型</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">幼师教案规范与幼儿观察记录指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 幼儿教师教案与观察记录专家');
...
const metricsList = [
  {
    "key": "kindergartenCurriculumStandard",
    "label": "五大领域教案规范"
  },
  {
    "key": "childObservationInsight",
    "label": "幼儿观察洞察深度"
  },
  {
    "key": "activitySafetyFeasibility",
    "label": "活动安全可行性"
  },
  {
    "key": "cornerDesignCreativity",
    "label": "区角环创创意度"
  },
  {
    "key": "homeSchoolCommunicationClarity",
    "label": "家园沟通清晰分"
  }
];
const guideRules = [
  {
    "title": "三维目标必须具体可衡量",
    "advice": "活动目标分为情感态度、认知理解及技能操作（如“能用语言说出水的 3 种用途”），避免过于宽泛",
    "avoid": "切忌使用“提高思想觉悟”等非幼儿语汇"
  },
  {
    "title": "观察记录重在客观描述",
    "advice": "记录观察到的具体言行（如“张某在建构区拿了 4 块红积木”），分析原因并给出下一步支持策略",
    "avoid": "避免充满主观偏见的断言（如“该幼儿性格霸道”）"
  },
  {
    "title": "材料投放注重层次性",
    "advice": "区角材料投放提供低中高 3 种难度（如建构区提供图纸与白纸），满足不同能力幼儿需求",
    "avoid": "严禁所有幼儿使用完全单一难度的材料"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[YOUSHI_SCORES\][\s\S]*?\[\/YOUSHI_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[YOUSHI_SCORES\](.*?)\[\/YOUSHI_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('youshi_history_records');
...
const saveHistory = () => {
  localStorage.setItem('youshi_history_records', JSON.stringify(historyList.value));
};
